...
| 占用存储 | 分表各自唯一，另用 `sg_map_cell (x, y)` 主键跨表互斥；选址时城 / 据点 / 市集都算占用 |
...
前端画布可按坐标生成平原 / 林 / 丘 / 山 / 水，叠加昼夜与天气、行军扬尘、流寇脉冲和名称标签；行军位置仍按 `departAt`～`arriveAt` 线性插值。地图页应比其他页更勤地刷新 `GET /api/world`，以便流寇与行军列表跟上结算。逻辑权威仍在服务端。
