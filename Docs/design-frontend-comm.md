...
- 城内 / 城外 / 城墙：场景舞台 + 点选建筑下令；卡片名册不再作为主界面。
- 登录 / 注册 / 创角 / 建城、商城与征兵：同一套昼夜场景舞台，点选铺面或校场兵种后再下令。
- 大地图：Canvas 拖拽、缩放、地形、昼夜天气、城点、据点、行军线与实时插值；不要用 DOM 画几千个格子。
- 音效：Web Audio 合成 UI / 建造 / 行军 / 环境声；需玩家首次点击后解锁，可静音。
